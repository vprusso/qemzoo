...
rendered from the JSON files under `data/`, so an edit is a JSON edit and the
site itself has no build step. Two small scripts derive `references.bib` and
`sitemap.xml` from that data.

To add a protocol:

1. Add its references to `data/references.json`, keyed by first author and year
   (then run `python3 scripts/generate_bibtex.py` and
   `python3 scripts/generate_sitemap.py`, which derive `references.bib` and
   `sitemap.xml` from the data)
...
- `references.bib` — BibTeX export of `data/references.json`, regenerate with `scripts/generate_bibtex.py`
